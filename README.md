...
I decided to add this, and it turns out that this causes all the src attributes have to re completely refetched and miss cache.nixos.org. It turns out that this causes a lot of hash mismatches. And then I went down this rabbit hole of trying to figure out all the src attributes that were unable to build for each pkgset. Turns out many of the pkgsets (like haskellPackages for example) barely have any 404s (probably because they're autogenerated). But others like python3Packages have 100+ missing source attributes. Crazy!
